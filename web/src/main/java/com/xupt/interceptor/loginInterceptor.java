...
public class loginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(request.getRequestURI().contains("/login") || request.getRequestURI().contains("/register") || request.getRequestURI().contains("/static"))
            return true;
        ...
        System.out.println("========================");
        try{
            Claims claims= Jwts.parser().setSigningKey("cereshuzhitingnizhenbangcereshuzhitingnizhenbang").parseClaimsJws(headerToken).getBody();
